...
// Cross-process mutex around the manifest file. gen:images, gen:videos and
// gen:unsplash (resolve-unsplash.ts) all read-modify-write asset-manifest.json;
// when the agent runs them in PARALLEL a stale in-memory snapshot overwrites
// the others' slots (observed build ca7371ff: the images write wiped the ~12
// Unsplash slots). O_EXCL ("wx") create is atomic across processes; resolve-
// unsplash.ts uses an identical lock on the SAME path so the two coordinate.
// Always re-read INSIDE the lock so the merge is against current disk state.
...
async function withManifestLock<T>(fn: () => Promise<T>): Promise<T> {
  ...
  // ~10s without acquiring → assume a crashed holder, clear and proceed.
...
export async function recordImage(slot: string, url: string): Promise<void> {
  await withManifestLock(async () => {
    const m = await readManifest();          // re-read under lock — never stale
    m.images[slot] = url;
